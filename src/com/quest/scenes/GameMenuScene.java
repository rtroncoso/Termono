...
public class GameMenuScene extends Scene{
	...
	public GameMenuScene(){
		...
		//Close
		this.mCloseSprite = new Sprite(Game.getSceneManager().getDisplay().getDisplayWidth() - 42, 100,this.mCloseTextureRegion,Game.getInstance().getVertexBufferObjectManager()) {					
					@Override
		...
		//##############FIN DE LA ENTIDAD PRINCIPAL#######################3
		
		/*
		
		//atachearlas cuando se llame a la tab
		this.mGameMenuEntity.attachChild(mInventoryEntity);
		
		this.mGameMenuEntity.attachChild(mEquipmentEntity);
		
		this.mGameMenuEntity.attachChild(mSkillsEntity);
		
		this.mGameMenuEntity.attachChild(mAttributesEntity);
		
		this.mGameMenuEntity.attachChild(mInfoEntity);
	*/
